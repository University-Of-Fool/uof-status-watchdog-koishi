import { Bot, clone, Context, Schema } from 'koishi'
import cron from 'koishi-plugin-cron'

export const name = 'uof-status-watchdog-koishi'
...
export const Config: Schema<Config> = Schema.object({
  broadcastGroup: Schema.string().description(`用于发送状态通知的群聊。`),
  broadcastErrorOnly: Schema.boolean().description(`开启后，只在有服务器出现异常时才会发送状态通知。`),
  detectInterval: Schema.number().description(`检测状态的时间间隔（1-60 分钟）。`),
  uofsServer: Schema.string().description(`UOF Status 服务端 API 地址`)
})
...
export async function apply(ctx: Context, config: Config) {
  ...
  ctx.middleware(async (session, next) => {
    ...
    async function broadcast(content: string) {
    ...
    ctx.cron(`*/${config.detectInterval} * * * *`, async () => {
      ...
      if (config.broadcastErrorOnly && isAllClientOnline(statusArray)) { return }
  ...
  async function getAllStatusArray() {
    ...
    for await (let i of _serverList) {
      ...
      let statusString = "离线"
      if (client.status === true) { statusString = "在线" }

      statusArray.push(
        { "name": i.name, "status": client.status, "statusString": statusString }
      )
    }
